...
This guide details how to migrate data from the existing GuestPass Database into the new `guestpass` schema in the ARDCity Database.

## Prerequisites
- Tools required: `pg_dump`, `psql` (or a GUI like pgAdmin/DBeaver)
- Credentials for both GuestPass DB (Source) and ARDCity DB (Target)
...
```
*Note: We only need `--data-only` because the schema structure is already created by `fsdcity_guestpass_schema.sql`.*
...
## Step 4: Import Data to ARDCity DB
Run the modified SQL against ARDCity DB.

**Command:**
```powershell
psql "postgres://user:pass@target_host/fsdcity_db?sslmode=require" -f guestpass_data.sql
```

## Step 5: Runtime Configuration
Update GuestPass application to use the ARDCity DB connection string with the search path option.

**.env file in GuestPass:**
```env
DATABASE_URL=postgres://fsdcity_user:password@neon-host/fsdcity_db?sslmode=require&options=-csearch_path%3Dguestpass,public
```
