...
export async function GET() {
  try {
    ...
    try {
      await sql`ALTER TABLE photo_logs ADD COLUMN IF NOT EXISTS photo_data JSONB DEFAULT '[]'::jsonb`;
...
export async function POST(request: Request) {
  try {
    ...
    const { project, location, photos, uploaded_by, time, tags, photo_data } = body;

    const result = await sql`
      INSERT INTO photo_logs (project, location, photos, uploaded_by, time, tags, photo_data)
      VALUES (${project}, ${location}, ${photos || 0}, ${uploaded_by}, ${time || 'Just now'}, ${tags || []}, ${JSON.stringify(photo_data || [])})
      RETURNING *
    `;
