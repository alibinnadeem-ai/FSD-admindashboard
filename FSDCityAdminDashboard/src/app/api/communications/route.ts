...
export async function GET() {
  try {
    ...
    try {
      await sql`ALTER TABLE communications ADD COLUMN IF NOT EXISTS reply_to INTEGER`;
    } catch (_) { /* column already exists */ }

    const communications = await sql`SELECT * FROM communications ORDER BY created_at ASC`;
...
export async function POST(request: Request) {
  try {
    ...
    const { project, user_name, message, reply_to } = body;
    ...
    const result = await sql`
      INSERT INTO communications (project, user_name, message, unread, reply_to)
      VALUES (${project}, ${user_name}, ${message}, ${1}, ${reply_to ?? null})
      RETURNING *
    `;
