      ...
      <input
        ...
        onChange={(e) => {
          if (e.target.files && e.target.files.length > 0) {
            setFormData({...formData, photoFiles: Array.from(e.target.files), photos: String(e.target.files.length)});
          }
        }}
      />
      {formData.photoFiles && (formData.photoFiles as File[]).length > 0 && (
        <p className="text-xs text-green-600 mt-1">{(formData.photoFiles as File[]).length} photo(s) selected</p>
      )}
    </div>
    <input
      type="text"
...
export const TaskForm: React.FC<FormProps> = ({ formData, setFormData, onSubmit, projects, isSubmitting }) => (
  <div className="space-y-4">
    <input
      type="text"
    ...
    <button
      ...
      className={`w-full py-2 rounded text-white ${isSubmitting ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}
    >
