...
export const useARDCityData = () => {
  ...
  const handleAddShift = async () => {
    const shiftData = {
      ...
      shift: formData.shift || 'Day (10AM-6PM)',
      status: 'Present',
  ...
  const handleAddPhotoLog = async () => {
    const files: File[] = formData.photoFiles || [];

    // Convert files to base64 data URLs
    const photoDataUrls: string[] = await Promise.all(
      files.map((file: File) => new Promise<string>((resolve, reject) => {
  ...
  const handleUpdateProjectStatus = async (id: number, status: Project['status']) => {
  ...
  const handleDownloadReport = async (id: number, format: string) => {
    ...
    const currentDownloads = report.downloads ?? 0;
    ...
    const reportTitle = `${report.type} - ${report.project}`;

    if (format === 'PDF') {
      // Generate a text-based report file
      const content = [
        '='.repeat(60),
        `GRAND CITY ADMIN DASHBOARD`,
        ...
        `On Duty: ${shifts.filter(s => s.status === 'On Duty' || s.status === 'Present').length}`,
        '',
        `Total Payments: PKR ${payments.reduce((sum, p) => sum + p.amount, 0).toLocaleString()}`,
        ...
        '-'.repeat(60),
        '',
        ...projects.map(p => `  ${p.name} | Status: ${p.status} | Progress: ${p.progress}% | Manager: ${p.manager}`),
        '',
        '-'.repeat(60),
        `Report generated by FSD City Admin Dashboard`,
        '='.repeat(60),
      ].join('\n');
      ...
      a.href = url;
      a.download = `${reportTitle.replace(/[^a-zA-Z0-9]/g, '_')}_${reportDate}.txt`;
      ...
      // Generate CSV
      const rows = [
        ['FSD City Admin Dashboard - ' + report.type],
        ['Project', report.project],
        ['Date', reportDate],
        [''],
        ['PROJECT SUMMARY'],
        ['Name', 'Status', 'Progress', 'Client', 'Manager', 'Team Size'],
        ...projects.map(p => [p.name, p.status, `${p.progress}%`, p.client, p.manager, String(p.team)]),
        [''],
        ['TASK SUMMARY'],
        ['Title', 'Assignee', 'Project', 'Priority', 'Due Date', 'Status', 'Completion'],
        ...tasks.map(t => [t.title, t.assignee, t.project, t.priority, t.due, t.status, `${t.completion}%`]),
        [''],
        ['PAYMENT SUMMARY'],
        ['Vendor', 'Amount (PKR)', 'Type', 'Due Date', 'Status', 'Project'],
        ...payments.map(p => [p.vendor, String(p.amount), p.type, p.due, p.status, p.project]),
      ];
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${reportTitle.replace(/[^a-zA-Z0-9]/g, '_')}_${reportDate}.csv`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
