...
export default function Scheduler() {
  ...
  const handleAcknowledge = async (maintenanceId: number, personnelId: number, isHR: boolean = false) => {
    ...
    try {
      // Build patch body — if an HR acknowledges and status is still pending, move to ongoing
      const patchBody: { acknowledgments: typeof updatedAcknowledgments; status?: string } = {
        acknowledgments: updatedAcknowledgments
      };
      if (isHR && maintenance.status === 'pending') {
        patchBody.status = 'ongoing';
      }
  ...
  const renderCalendar = () => {
    ...
    for (let i = 1; i <= lastDay.getDate(); i++) {
      ...
      days.push(
        <div
          key={i}
          className={`min-h-20 p-2 border border-gray-200 cursor-pointer transition-all hover:border-blue-500 hover:shadow-md ${isToday ? 'bg-blue-500 text-white' :
            selectedDate === `${year}-${String(month + 1).padStart(2, '0')}-${String(i).padStart(2, '0')}` ? 'bg-blue-100 border-blue-400' :
            'bg-white'
            }`}
          onClick={() => {
  ...
  const renderUpcoming = () => {
    // If a date is selected, show tasks for that specific date
    if (selectedDate) {
      const dateTasks = maintenances.filter(m => {
        const mDateStr = m.date ? m.date.split('T')[0] : '';
      ...
      if (dateTasks.length === 0) {
        return <div className="text-center text-gray-500 py-8">No tasks scheduled for this date</div>;
      }

      return dateTasks.map(m => (
        <div
          key={m.id}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {dayDetailDate && (() => {
        const dateLabel = new Date(dayDetailDate + 'T00:00:00').toLocaleDateString('en-US', {
          weekday: 'long', year: 'numeric', month: 'long', day: 'numeric'
        });
        const dayTasks = maintenances.filter(m => {
          const mDateStr = m.date ? m.date.split('T')[0] : '';
          return mDateStr === dayDetailDate;
        });
        return (
          <Modal title={`Tasks — ${dateLabel}`} onClose={() => setDayDetailDate(null)}>
            <div className="p-6">
              <h2 className="text-xl font-bold mb-1">All Tasks</h2>
              <p className="text-sm text-gray-500 mb-4">{dateLabel}</p>

              {dayTasks.length === 0 ? (
                <div className="text-center text-gray-500 py-10">No tasks scheduled for this date.</div>
              ) : (
                <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
                  {dayTasks.map(m => (
                    <div
                      key={m.id}
                      className="p-4 border border-gray-200 rounded-lg cursor-pointer hover:border-blue-400 hover:shadow-sm transition-all"
                      onClick={() => {
                        setSelectedMaintenance(m);
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="font-semibold text-gray-900">{m.type}</div>
                          <div className="text-sm text-gray-600 mt-0.5">{m.property_name}</div>
                          {m.property_location && (
                            <div className="text-xs text-gray-400">{m.property_location}</div>
                          )}
                        </div>
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          ...
                          {m.priority === 'Urgent' && (
                            <span className="px-2 py-0.5 text-xs rounded font-medium bg-red-100 text-red-800">URGENT</span>
                          )}
                        </div>
                      </div>
                      <div className="mt-2 text-sm text-gray-600 flex flex-wrap gap-x-4 gap-y-1">
                        <span>Vendor: {m.vendor_name}</span>
                        <span>Time: {m.requested_time}</span>
                      </div>
                      {m.description && (
                        <div className="mt-1 text-xs text-gray-500 line-clamp-2">{m.description}</div>
                      )}
                    </div>
                  ))}
                </div>
