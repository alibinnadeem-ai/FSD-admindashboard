...
import { Project, Communication } from '../types';
import { useARDCityData } from '../hooks/useARDCityData';
...
export default function Dashboard() {
  ...
  const [viewedComm, setViewedComm] = useState<Communication | null>(null);
  ...
  const renderModalContent = () => {
    ...
    switch (modalType) {
      ...
      case 'task':
        return <TaskForm {...formProps} onSubmit={handleAddTask} isSubmitting={isSubmittingTask} />;
      case 'payment':
  ...
  return (
    <div>
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-md">
            <ul className="divide-y divide-gray-200">
              {communications
                ...
                .map((comm) => {
                  const replies = communications
                    .filter(c => c.reply_to === comm.id)
                    ...
                    <li key={comm.id} className="px-6 py-4">
                      <div className="flex items-start justify-between">
                        <div className="flex-1 min-w-0">
                          ...
                          {/* Thread replies */}
                          {replies.length > 0 && (
                            <div className="mt-3 ml-4 pl-4 border-l-2 border-blue-200 space-y-2">
                              {replies.map(reply => (
                                <div key={reply.id} className="bg-blue-50 rounded p-2">
                                  <p className="text-sm font-medium text-gray-900">{reply.user}</p>
                                  <p className="text-sm text-gray-600">{reply.message}</p>
                                  <p className="text-xs text-gray-400">{formatRelativeTime(reply.time)}</p>
                                </div>
                              ))}
                            </div>
                          )}
                          ...
                          {openCommId === comm.id && (
                            <div className="mt-3 space-y-2">
                              ...
                              <div className="flex items-center gap-2">
                                    ...
                                    try {
                                      ...
                                      if (res.ok) {
                                        ...
                                        setCommunications(prev => [...prev, {
                                          ...
                                          time: newComm.created_at,
                                          unread: newComm.unread || 0,
                                          reply_to: newComm.reply_to ?? null
                                        }]);
                                      }
                                    } catch (err) {
                                ...
                                <button
                                  onClick={() => { setOpenCommId(null); setReplyName(''); setReplyText(''); }}
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="mt-2">
                          ...
                          {openPhotoId === log.id && (
                            <div className="mt-2">
                              {log.photo_data && log.photo_data.length > 0 ? (
                                <div className="grid grid-cols-3 gap-2 mt-2">
                                  {log.photo_data.map((src, idx) => (
                                    <img
                                      key={idx}
                                      src={src}
                                      alt={`Photo ${idx + 1}`}
                                      className="w-full h-24 object-cover rounded border border-gray-200 hover:opacity-80 cursor-pointer"
                                      onClick={() => setExpandedPhoto(src)}
                                    />
                                  ))}
                                </div>
                              ) : (
                                <p className="mt-2 text-xs text-gray-500">No photos uploaded for this log.</p>
                              )}
                            </div>
                          )}
                        </div>
      ...
      {viewedComm && (() => {
        const replies = communications
          .filter(c => c.reply_to === viewedComm.id)
          .sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime());
        return (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg max-w-lg w-full max-h-[80vh] flex flex-col">
              <div className="p-4 border-b border-gray-200 flex justify-between items-center">
                ...
                <button onClick={() => setViewedComm(null)} className="text-gray-500 hover:text-gray-700">
                  ✕
                </button>
              </div>
              <div className="p-4 overflow-y-auto flex-1">
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-sm font-semibold text-gray-900">{viewedComm.user}</p>
                    <p className="text-xs text-gray-400">{formatRelativeTime(viewedComm.time)}</p>
                  </div>
                  <p className="text-xs text-gray-500 mb-2">{viewedComm.project}</p>
                  <p className="text-sm text-gray-700 whitespace-pre-wrap">{viewedComm.message}</p>
                </div>
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const res = await fetch('/api/communications', {
                          ...
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({
                            project: viewedComm.project,
                            user_name: replyName.trim() || 'You',
                            message: replyText.trim(),
                            reply_to: viewedComm.id
                          })
                        });
                        if (res.ok) {
                          const newComm = await res.json();
